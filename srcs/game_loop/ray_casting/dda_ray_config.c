...
void	get_ray_config_dda(t_ray *ray)
{
	...
	if (ray->dir.x < 0)
	{
		...
		ray->side_dist.x = (ray->true_pos.x - ray->grid_pos.x)
			* ray->delta_dist.x;
	}
	else
	{
		ray->step_direction.x = 1;
		ray->side_dist.x = (ray->grid_pos.x + 1.0 - ray->true_pos.x)
			* ray->delta_dist.x;
	}
	if (ray->dir.y < 0)
	{
		ray->step_direction.y = -1;
		ray->side_dist.y = (ray->true_pos.y - ray->grid_pos.y)
			* ray->delta_dist.y;
	}
	else
	{
		ray->step_direction.y = 1;
		ray->side_dist.y = (ray->grid_pos.y + 1.0 - ray->true_pos.y)
			* ray->delta_dist.y;
	}
}
